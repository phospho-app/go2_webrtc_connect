...
def display_data(message):
    ...
    # IMU State (RPY)
    print(
        f"IMU - RPY: Roll: {imu_state[0]}, Pitch: {imu_state[1]}, Yaw: {imu_state[2]}"
    )
    ...
    for i, motor in enumerate(motor_state):
        ...
        print(
            f"Motor {i + 1:2}: q={motor['q']:.4f}, Temp={motor['temperature']}°C, Lost={motor['lost']}"
        )
...
async def main():
    try:
        ...
        conn = Go2WebRTCConnection(WebRTCConnectionMethod.LocalSTA, ip="192.168.1.193")
        ...
        print("Subscribing to lowstate data...")
        await conn.connect()
        print("Subscribing to lowstate data...")

...
class WebRTCDataChannelNetworkStatus:
    ...
    def handle_response(self, info):
        ...
        if (
            ...
            or status == "NetworkStatus.ON_WIFI_CONNECTED"
        ):
...
class WebRTCDataChannelFileUploader:
    ...
    async def upload_file(
        self, data, file_path, chunk_size=60 * 1024, progress_callback=None
    ):
...
class WebRTCDataChannelFileDownloader:
    ...
    async def download_file(
        self, file_path, chunk_size=60 * 1024, progress_callback=None
    ):
        ...
        try:
            ...
            response = await self.publish(
                "", request_message, DATA_CHANNEL_TYPE["RTC_INNER_REQ"]
            )
...
class WebRTCDataChannelRTCInnerReq:
    def __init__(self, conn, channel, pub_sub):
        ...
        self.network_status = WebRTCDataChannelNetworkStatus(
            self.conn, self.channel, pub_sub
        )

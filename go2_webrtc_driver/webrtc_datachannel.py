...
class WebRTCDataChannel:
    def __init__(self, conn, pc) -> None:
        ...
        self.rtc_inner_req = WebRTCDataChannelRTCInnerReq(
            self.conn, self.channel, self.pub_sub
        )
        ...
        self.rtc_inner_req.network_status.set_on_network_status_callback(
            on_network_status
        )
        ...
        async def on_message(message):
            logging.info(f"Received message on data channel: {message}")
            try:
                ...
                logging.error(
                    f"Failed to decode JSON message: {message}", exc_info=True
                )
